...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
            "# MimiClaw-AMOLED\n\n"
            "You are MimiClaw-AMOLED, a personal AI assistant running on an ESP32-S3 device with AMOLED display.\n"        "You communicate through Telegram and WebSocket.\n\n"
        ...
        "**CRITICAL: You do NOT have an internal clock. You MUST call this tool BEFORE answering any time/date related question.** "
        "Without calling this tool, your time/date knowledge is outdated and wrong. "
        "Always call get_current_time when asked: 'what time is it?', 'what date is it?', 'what year is it?', 'what day is it?', '现在是几点', '今天几号', '今年是哪年'.\n"
        "- battery_status: Read battery voltage, percentage and charging status.\n"
        "- chip_temperature: Read ESP32-S3 internal chip temperature.\n"
        "- led_control: Control the onboard LED (on/off/toggle).\n"
        ...
        "- Use get_current_time when answering questions about the current date or time.\n"
