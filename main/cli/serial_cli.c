...
static int cmd_set_model_provider(int argc, char **argv)
{
    ...
    const char *provider = provider_args.provider->sval[0];
    
    /* Validate provider name */
    if (strcmp(provider, "anthropic") != 0 &&
        strcmp(provider, "openai") != 0 &&
        strcmp(provider, "openrouter") != 0 &&
        strcmp(provider, "ollama") != 0 &&
        strcmp(provider, "custom") != 0) {
        printf("Error: Invalid provider '%s'\n", provider);
        printf("Valid providers: anthropic, openai, openrouter, ollama, custom\n");
        printf("Note: Use provider NAME, not URL!\n");
        return 1;
    }
    
    llm_set_provider(provider);
    printf("Provider set to '%s'. Restart to apply.\n", provider);
    return 0;
}
...
static int cmd_set_ollama(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&ollama_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, ollama_args.end, argv[0]);
        return 1;
    }
    
    const char *host = ollama_args.host->sval[0];
    const char *port = ollama_args.port->count > 0 ? ollama_args.port->sval[0] : "11434";
    const char *model = ollama_args.model->count > 0 ? ollama_args.model->sval[0] : "llama3.2";
    
    llm_set_ollama_host(host);
    llm_set_ollama_port(port);
    llm_set_ollama_model(model);
    llm_set_provider("ollama");  /* Auto-switch to Ollama provider */
    
    printf("Ollama configured: http://%s:%s (model: %s)\n", host, port, model);
    printf("Provider set to 'ollama'. Restart to apply.\n");
    return 0;
}
...
static int cmd_set_api_url(int argc, char **argv)
{
    ...
    const char *url = api_url_args.url->sval[0];
    const char *host = api_url_args.host->count > 0 ? api_url_args.host->sval[0] : NULL;
    
    /* Validate URL starts with https:// or http:// */
    if (strncmp(url, "https://", 8) != 0 && strncmp(url, "http://", 7) != 0) {
        printf("Error: URL must start with https:// or http://\n");
        return 1;
    }
    
    /* Check if URL contains /chat/completions or similar endpoint */
    if (strstr(url, "/chat/completions") == NULL && strstr(url, "/v1/messages") == NULL) {
        printf("Warning: URL may be incomplete.\n");
        printf("For OpenAI-compatible APIs, URL should end with /chat/completions\n");
        printf("Example: https://api.deepseek.com/v1/chat/completions\n");
    }
    
    llm_set_api_url(url);
    if (host) {
        llm_set_api_host(host);
    }
    llm_set_provider("custom");  /* Auto-switch to custom provider */
    
    printf("Custom API configured: %s\n", url);
    if (host) {
        printf("API host (TLS): %s\n", host);
    }
    printf("Provider set to 'custom'. Restart to apply.\n");
    return 0;
}
...
static int cmd_config_show(int argc, char **argv)
{
    ...
    print_config("Ollama Host", MIMI_NVS_LLM,   MIMI_NVS_KEY_OLLAMA_HOST, MIMI_SECRET_OLLAMA_HOST, false);
    print_config("Ollama Port", MIMI_NVS_LLM,   MIMI_NVS_KEY_OLLAMA_PORT, MIMI_SECRET_OLLAMA_PORT, false);
    print_config("Ollama Model", MIMI_NVS_LLM,  MIMI_NVS_KEY_OLLAMA_MODEL, MIMI_SECRET_OLLAMA_MODEL, false);
    print_config("Custom URL",  MIMI_NVS_LLM,   MIMI_NVS_KEY_API_URL,  MIMI_SECRET_API_URL,    false);
    print_config("Custom Host", MIMI_NVS_LLM,   MIMI_NVS_KEY_API_HOST, MIMI_SECRET_API_HOST,   false);
...
/* --- ask command --- */
static struct {
    struct arg_str *question;
    struct arg_end *end;
} ask_args;

static int cmd_ask(int argc, char **argv)
{
    int nerrors = arg_parse(argc, argv, (void **)&ask_args);
    if (nerrors != 0) {
        arg_print_errors(stderr, ask_args.end, argv[0]);
        return 1;
    }
    
    mimi_msg_t msg = {
        .channel = MIMI_CHAN_CLI,
        .chat_id = "cli",
        .content = strdup(ask_args.question->sval[0]),
    };
    ...
    printf("Message sent to agent: %s\n", ask_args.question->sval[0]);
    return 0;
}
...
esp_err_t serial_cli_init(void)
{
    ...
    /* set_model_provider */
    provider_args.provider = arg_str1(NULL, NULL, "<provider>", "Provider name: anthropic|openai|openrouter|ollama|custom");
    provider_args.end = arg_end(1);
    esp_console_cmd_t provider_cmd = {
        .command = "set_model_provider",
        .help = "Set LLM provider name (NOT URL, just: anthropic/openai/openrouter/ollama/custom)",
    ...
    /* set_ollama */
    ollama_args.host = arg_str1(NULL, NULL, "<host>", "Ollama server IP (e.g. 192.168.1.100)");
    ollama_args.port = arg_str0(NULL, NULL, "<port>", "Ollama port (default: 11434)");
    ollama_args.model = arg_str0(NULL, NULL, "<model>", "Ollama model (default: llama3.2)");
    ollama_args.end = arg_end(3);
    esp_console_cmd_t ollama_cmd = {
        .command = "set_ollama",
        .help = "Configure Ollama server (host [port] [model])",
        .func = &cmd_set_ollama,
        .argtable = &ollama_args,
    };
    esp_console_cmd_register(&ollama_cmd);

    /* set_api_url */
    api_url_args.url = arg_str1(NULL, NULL, "<url>", "Full API URL (e.g. https://api.deepseek.com/v1/chat/completions)");
    api_url_args.host = arg_str0(NULL, NULL, "<host>", "API host for TLS (e.g. api.deepseek.com)");
    api_url_args.end = arg_end(2);
    esp_console_cmd_t api_url_cmd = {
        .command = "set_api_url",
        .help = "Configure custom API URL (auto-switches to 'custom' provider)",
    ...
    /* ask */
    ask_args.question = arg_str1(NULL, NULL, "<question>", "Question to ask the AI");
    ask_args.end = arg_end(1);
    esp_console_cmd_t ask_cmd = {
        .command = "ask",
        .help = "Ask a question to the AI (e.g. ask \"What is ESP32?\")",
        .func = &cmd_ask,
        .argtable = &ask_args,
    };
