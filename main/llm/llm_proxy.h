 ...
 * @return Provider name string (e.g. "anthropic", "openai", "ollama")
 */
...
/**
 * Save Ollama server host to NVS.
 */
esp_err_t llm_set_ollama_host(const char *host);

/**
 * Save Ollama server port to NVS.
 */
esp_err_t llm_set_ollama_port(const char *port);

/**
 * Save Ollama model name to NVS.
 */
esp_err_t llm_set_ollama_model(const char *model);

/**
 * Save custom API URL to NVS (for 'custom' provider).
 */
esp_err_t llm_set_api_url(const char *url);

/**
 * Save custom API host to NVS (for 'custom' provider, used for TLS).
 */
