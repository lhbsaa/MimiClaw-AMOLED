...
#define LLM_OLLAMA_HOST_MAX_LEN  48
#define LLM_OLLAMA_PORT_MAX_LEN  8
#define LLM_API_URL_MAX_LEN      256
#define LLM_API_HOST_MAX_LEN     64

static char s_api_key[LLM_API_KEY_MAX_LEN] = {0};
static char s_model[LLM_MODEL_MAX_LEN] = MIMI_LLM_DEFAULT_MODEL;
static char s_provider[16] = MIMI_LLM_PROVIDER_DEFAULT;
static char s_ollama_host[LLM_OLLAMA_HOST_MAX_LEN] = MIMI_SECRET_OLLAMA_HOST;
static char s_ollama_port[LLM_OLLAMA_PORT_MAX_LEN] = MIMI_SECRET_OLLAMA_PORT;
static char s_api_url[LLM_API_URL_MAX_LEN] = MIMI_SECRET_API_URL;
static char s_api_host[LLM_API_HOST_MAX_LEN] = MIMI_SECRET_API_HOST;
...
{
    return strcmp(s_provider, "openai") == 0 || strcmp(s_provider, "openrouter") == 0;
}
...
{
    static char ollama_url[128];
    if (provider_is_custom()) {
        return s_api_url[0] ? s_api_url : "";
    }
    if (provider_is_openrouter()) {
        return "https://openrouter.ai/api/v1/chat/completions";
    }
    if (provider_is_ollama()) {
        snprintf(ollama_url, sizeof(ollama_url), "http://%s:%s/v1/chat/completions",
                 s_ollama_host, s_ollama_port);
        return ollama_url;
    }
...
{
    static char ollama_host[64];
    if (provider_is_custom()) {
        return s_api_host[0] ? s_api_host : "";
    }
    if (provider_is_openrouter()) {
        return "openrouter.ai";
    }
    if (provider_is_ollama()) {
        snprintf(ollama_host, sizeof(ollama_host), "%s:%s", s_ollama_host, s_ollama_port);
        return ollama_host;
    }
...
{
    /* Custom, Ollama and OpenAI use the same path */
...
esp_err_t llm_proxy_init(void)
{
    ...
    if (nvs_open(MIMI_NVS_LLM, NVS_READONLY, &nvs) == ESP_OK) {
        ...
        /* Ollama config */
        char ollama_host_tmp[LLM_OLLAMA_HOST_MAX_LEN] = {0};
        len = sizeof(ollama_host_tmp);
        if (nvs_get_str(nvs, MIMI_NVS_KEY_OLLAMA_HOST, ollama_host_tmp, &len) == ESP_OK && ollama_host_tmp[0]) {
            safe_copy(s_ollama_host, sizeof(s_ollama_host), ollama_host_tmp);
        }
        char ollama_port_tmp[LLM_OLLAMA_PORT_MAX_LEN] = {0};
        len = sizeof(ollama_port_tmp);
        if (nvs_get_str(nvs, MIMI_NVS_KEY_OLLAMA_PORT, ollama_port_tmp, &len) == ESP_OK && ollama_port_tmp[0]) {
            safe_copy(s_ollama_port, sizeof(s_ollama_port), ollama_port_tmp);
        }
        char ollama_model_tmp[LLM_MODEL_MAX_LEN] = {0};
        len = sizeof(ollama_model_tmp);
        if (nvs_get_str(nvs, MIMI_NVS_KEY_OLLAMA_MODEL, ollama_model_tmp, &len) == ESP_OK && ollama_model_tmp[0]) {
            /* If using Ollama provider, use Ollama model */
            if (strcmp(s_provider, "ollama") == 0) {
                safe_copy(s_model, sizeof(s_model), ollama_model_tmp);
            }
        }
        /* Custom API config */
        ...
        char api_host_tmp[LLM_API_HOST_MAX_LEN] = {0};
        len = sizeof(api_host_tmp);
        if (nvs_get_str(nvs, MIMI_NVS_KEY_API_HOST, api_host_tmp, &len) == ESP_OK && api_host_tmp[0]) {
            safe_copy(s_api_host, sizeof(s_api_host), api_host_tmp);
        }
        nvs_close(nvs);
    }
    ...
    if (s_api_key[0] || provider_is_ollama()) {
        ...
        if (provider_is_ollama()) {
            ESP_LOGI(TAG, "Ollama server: %s:%s", s_ollama_host, s_ollama_port);
        }
        if (provider_is_custom()) {
            ESP_LOGI(TAG, "Custom API: %s", s_api_url);
        }
    } else {
...
static esp_err_t llm_http_direct(const char *post_data, resp_buf_t *rb, int *out_status)
{
    ...
    /* Ollama uses HTTP, others use HTTPS with cert verification */
    if (!provider_is_ollama()) {
    ...
    if (provider_uses_openai_format()) {
        ...
        if (provider_is_openrouter()) {
                    esp_http_client_set_header(client, "HTTP-Referer", "https://github.com/lhbsaa/MimiClaw-AMOLED");
                    esp_http_client_set_header(client, "X-Title", "MimiClaw-AMOLED");        }
    } else {
        /* Anthropic format */
                         ...
                         llm_response_t *resp)
{
    ...
    /* OpenAI/Ollama/Custom format */
    if (provider_uses_openai_format()) {
        /* Ollama, OpenRouter and Custom use max_tokens, OpenAI uses max_completion_tokens */
        if (provider_is_ollama() || provider_is_openrouter() || provider_is_custom()) {
    ...
    /* OpenAI/Ollama/Custom format (choices array) */
    if (provider_uses_openai_format()) {
...
esp_err_t llm_set_api_key(const char *api_key)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to set API key: %s", esp_err_to_name(err));
        nvs_close(nvs);
        return err;
    }
    err = nvs_commit(nvs);
...
esp_err_t llm_set_ollama_host(const char *host)
{
    ...
    esp_err_t err = nvs_open(MIMI_NVS_LLM, NVS_READWRITE, &nvs);
    if (err != ESP_OK) return err;
    err = nvs_set_str(nvs, MIMI_NVS_KEY_OLLAMA_HOST, host);
    if (err != ESP_OK) {
        nvs_close(nvs);
        return err;
    }
...
esp_err_t llm_set_ollama_port(const char *port)
{
    ...
    esp_err_t err = nvs_open(MIMI_NVS_LLM, NVS_READWRITE, &nvs);
    if (err != ESP_OK) return err;
    err = nvs_set_str(nvs, MIMI_NVS_KEY_OLLAMA_PORT, port);
    if (err != ESP_OK) {
        nvs_close(nvs);
        return err;
    }
...
esp_err_t llm_set_ollama_model(const char *model)
{
    nvs_handle_t nvs;
    esp_err_t err = nvs_open(MIMI_NVS_LLM, NVS_READWRITE, &nvs);
    if (err != ESP_OK) return err;
    err = nvs_set_str(nvs, MIMI_NVS_KEY_OLLAMA_MODEL, model);
    if (err != ESP_OK) {
        nvs_close(nvs);
        return err;
    }
    ...
    /* If using Ollama provider, update the model */
    if (strcmp(s_provider, "ollama") == 0) {
        safe_copy(s_model, sizeof(s_model), model);
    }
    ESP_LOGI(TAG, "Ollama model set to: %s", model);
    return ESP_OK;
}
...
esp_err_t llm_set_api_url(const char *url)
{
    ...
    ESP_LOGI(TAG, "Custom API URL set to: %s", s_api_url);
    return ESP_OK;
}

esp_err_t llm_set_api_host(const char *host)
{
    nvs_handle_t nvs;
    esp_err_t err = nvs_open(MIMI_NVS_LLM, NVS_READWRITE, &nvs);
    if (err != ESP_OK) return err;
    err = nvs_set_str(nvs, MIMI_NVS_KEY_API_HOST, host);
    if (err != ESP_OK) {
        nvs_close(nvs);
        return err;
    }
    ...
    safe_copy(s_api_host, sizeof(s_api_host), host);
    ESP_LOGI(TAG, "Custom API host set to: %s", s_api_host);
    return ESP_OK;
}
